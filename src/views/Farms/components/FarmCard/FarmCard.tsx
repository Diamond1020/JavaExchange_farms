...
import { getMasterChefAddressJava } from 'utils/addressHelpers'
...
import useWithdrawalFeeTimer from '../hooks/useWithdrawalDelayTimer'
import WithdrawalDelayTimer from './WithDrawDelayTimer'
...
const FarmCard: React.FC<FarmCardProps> = ({ farm, removed, cakePrice, bnbPrice, javaPrice, ethereum, account }) => {
  ...
  const [poolinfo, setPoolInfo] = useState({ lockTime: new BigNumber(0)});
 ...
 //  const farmImage = farm.isTokenOnly ? farm.tokenSymbol.toLowerCase() : `${farm.tokenSymbol.toLowerCase()}-${farm.quoteTokenSymbol.toLowerCase()}`
  ...
  const earnToken = farm.earnToken
  ...
  useEffect(() => {
    ...
    if (earnToken === 'JAVA' && account != null) {
      fetchAllBalances();
    }
  }, [account, earnToken, farm])

  const { hasUnstakingFee, secondsRemaining} = useWithdrawalFeeTimer(
    Number(userinfo.depositTime),
    userinfo.amount,
  ...
  return (
    <FCard>
      ...
      <Flex justifyContent='space-between'>
        <Text bold style={{ fontSize: '12px' }}>{(farm.withdrawFeeBP / 100)}% unstaking fee</Text>
      </Flex>
      }
      ...
      <Text fontSize="14px"> {withdrawalDelayPeriod} lock time</Text>
      }  
